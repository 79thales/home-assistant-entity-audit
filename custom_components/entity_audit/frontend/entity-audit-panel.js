class EntityAuditPanel extends HTMLElement {
  ...
  _automationYaml(configs) {
    const snapshots = configs.map(({ item, config }) => [
    ...
    return `${this._configurationHeader()}${snapshots.join("\n\n")}\n`;
  }
  ...
  async _exportAutomationPackage() {
    ...
    try {
      ...
      const configuration = this._configurationIncludeYaml();
      const readme = [
      ...
      <ha-top-app-bar-fixed ${this._narrow ? "narrow" : ""}>
        ...
        <main>
          ...
          <section class="filter-panel ${this._filtersOpen ? "open" : ""}">
            ...
            <p class="privacy-note">Click an item name to open its native Home Assistant editor. CSV includes the current enabled/disabled status for automations. YAML exports use Home Assistant's administrator-only configuration API for the displayed items. Export configuration.yaml downloads a safe include snippet, not the complete server configuration. The ZIP package contains all available automations, scripts, and the same include snippet. Sensitive values are redacted before download.</p>
          </section>
...
if (!customElements.get("entity-audit-panel-v0321")) {
  customElements.define("entity-audit-panel-v0321", EntityAuditPanel);
}
